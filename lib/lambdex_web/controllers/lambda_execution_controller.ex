defmodule LambdexWeb.LambdaExecutionController do
  ...
  def create(conn, %{"lambda_execution" => lambda_execution_params}) do
    with {:ok, %LambdaExecution{} = lambda_execution} <- Lambdas.create_lambda_execution(lambda_execution_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "lambda_execution" => lambda_execution_params}) do
    ...
    with {:ok, %LambdaExecution{} = lambda_execution} <- Lambdas.update_lambda_execution(lambda_execution, lambda_execution_params) do
      render(conn, "show.json", lambda_execution: lambda_execution)
    end
  end
