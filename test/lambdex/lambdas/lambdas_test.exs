defmodule Lambdex.LambdasTest do
  use Lambdex.DataCase
  ...
  describe "lambdas" do
    ...
    @update_attrs %{code: "some updated code", enabled: false, params: %{}, path: "some updated path"}
  ...
  describe "lambda_executions" do
    ...
    test "create_lambda_execution/1 with valid data creates a lambda_execution" do
      assert {:ok, %LambdaExecution{} = lambda_execution} = Lambdas.create_lambda_execution(@valid_attrs)
    ...
    test "update_lambda_execution/2 with valid data updates the lambda_execution" do
      lambda_execution = lambda_execution_fixture()
      assert {:ok, %LambdaExecution{} = lambda_execution} = Lambdas.update_lambda_execution(lambda_execution, @update_attrs)
      assert lambda_execution.data == %{}
    end

    test "update_lambda_execution/2 with invalid data returns error changeset" do
      lambda_execution = lambda_execution_fixture()
      assert {:error, %Ecto.Changeset{}} = Lambdas.update_lambda_execution(lambda_execution, @invalid_attrs)
    ...
    test "delete_lambda_execution/1 deletes the lambda_execution" do
      ...
      assert_raise Ecto.NoResultsError, fn -> Lambdas.get_lambda_execution!(lambda_execution.id) end
    end
